...
async function signInAsHost(context: BrowserContext, page: Page) {
  const response = await context.request.post("/api/test/session", {
    data: { uid: "host-e2e", name: "Ash", email: "ash@example.com" },
  });
...
async function startOrder(page: Page): Promise<string> {
  await page.getByRole("button", { name: "Start a group order" }).click();
  await page.waitForURL(/\/orders\/.+/);
  const match = page.url().match(/\/orders\/([^/]+)/);
  return match![1];
}

async function inviteGuest(page: Page): Promise<string> {
  await page.getByPlaceholder("friend@example.com").fill("misty@example.com");
  await page.getByRole("button", { name: "Invite" }).click();
  const link = page.locator("code", { hasText: "/join/" });
...
test("host runs a group order end to end", async ({ browser }) => {
  ...
  await signInAsHost(hostContext, hostPage);
  const orderId = await startOrder(hostPage);
  const joinUrl = await inviteGuest(hostPage);
